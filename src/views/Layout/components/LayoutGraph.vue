<template>
    ...
    <el-drawer
      ...
      >
        ...
        <div v-if="selectedNode.imageurl">
            <img
                :src="getImageUrl(selectedNode.imageurl)"
                alt="Node Image"
                style="width: 100%; height: auto; margin-bottom: 20px;"
                @dblclick="openImageViewer(selectedNode.imageurl)"
            />
        </div>

        <div v-if="selectedNode.videourl">
            <video
            :src="getVideoUrl(selectedNode.videourl)"
            controls
...
export default {
  ...
  data() {
    return {
      ...
      imageViewerVisible: false, // 图片查看器的显示状态
      currentImageUrl: "", // 当前查看的图片 URL
    };
  },
  mounted() {
    this.initData();
...
.on('dblclick', (event, d) => {
    ...
    this.selectedNode.imageurl = this.findNodeImageURLById(this.selectedNode.id);
    this.selectedNode.videourl = this.findNodeVideoURLById(this.selectedNode.id);
...
.attr('flood-color', '#888'); // 阴影颜色
    ...
    {
        this.expandNode2(item.end_properties.id);
    }
    },
    unshowAllNodes() {
  // 将所有节点的 visible 属性设置为 false
  this.nodesData.forEach(node => {
    node.visible = false;
  });

  // 将所有边的 visible 属性设置为 false
...
inupdateGraph() {
  ...
  this.linksText.attr('visibility', d => d.visible ? 'visible' : 'hidden');
    ...
    //   // 遍历 response 数据，查找与 nodeId 匹配的节点Mode
    //   for (const item of this.response) {
    ...
    //   return null;
    // },
    findNodeImageURLById(nodeId) {
      // 遍历 response 数据，查找与 nodeId 匹配的节点URL
      for (const item of this.response) {
        if (item.end_properties.id === nodeId && item.start_properties.URL && item.start_properties.Mode === '图片') {
          return item.start_properties.URL;
        }
      }
      return null;
    },
    findNodeVideoURLById(nodeId) {
      // 遍历 response 数据，查找与 nodeId 匹配的节点URL
      for (const item of this.response) {
        if (item.end_properties.id === nodeId && item.start_properties.URL && item.start_properties.Mode === '视频') {
          return item.start_properties.URL;
        }
      }
      return null;
...
expandNode2(nodeid) {
  // 获取以当前节点为终点的所有边，并且这些边当前是隐藏的
  const edgesToShow = this.edges.filter(edge => edge.target.id === nodeid && !edge.visible&&  edge.relation !== 'Next_P' && edge.relation !== 'Next_SS');
  ...
  edgesToShow.forEach(edge => {
    ...
    targetNode.visible = true; // 更新节点的 visible 属性
...
showNode(node) {
  ...
  handleAction(action) {
    ...
    this.hideNodeRecursive(sourceNode);
      ...
      console.log('Action 1 trigxgered!');
    },

    Staylevel1() {
    const nodes=this. nodes.filter(node => node.weight === 5);
      this.showAllNodes();
    ...
    Staylevel2() {
      const nodes=this. nodes.filter(node => node.weight === 4);
      this.showAllNodes();
      for( const node of nodes)
      {
        this.collapseNode(node);
    ...
    Staylevel3() {
        const nodes=this. nodes.filter(node => node.weight === 3);
      this.showAllNodes();
      for( const node of nodes)
      {
        this.collapseNode(node);
